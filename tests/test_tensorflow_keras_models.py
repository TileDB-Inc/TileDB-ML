...
from tiledb.ml.models.tensorflow import TensorflowTileDBModel
...
def test_load_tiledb_error_with_wrong_uri():
    tiledb_model_obj = TensorflowTileDBModel(uri="dummy_uri")
...
class TestTensorflowKerasModel:
    def test_save_model_to_tiledb_array(self, tmpdir, api, loss, optimizer, metrics):
        ...
        tiledb_model_obj = TensorflowTileDBModel(uri=tiledb_uri)
    ...
    def test_preview(self, tmpdir, api, loss, optimizer, metrics):
        ...
        tiledb_model_obj = TensorflowTileDBModel(uri=tiledb_uri, model=model)
        if model.built:
...
def test_file_properties(tmpdir):
    ...
    tiledb_obj = TensorflowTileDBModel(uri=tiledb_array)
...
def test_file_properties_in_tiledb_cloud_case(tmpdir, mocker):
    ...
    tiledb_array = os.path.join(tmpdir, "model_array")
    tiledb_obj = TensorflowTileDBModel(uri=tiledb_array, namespace="test_namespace")
    tiledb_obj.save(model=model)
