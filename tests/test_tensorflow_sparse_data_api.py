...
def create_model(request) -> object:
    model = Sequential()
...
class TestTileDBTensorflowSparseDataAPI:
    ...
    def test_sparse_except_with_diff_number_of_x_y_rows(self, temp_uri, create_model):
        with temp_uri as temp_dir:
            array_uuid = str(uuid.uuid4())
            tiledb_uri_x = os.path.join(temp_dir, "x" + array_uuid)
            tiledb_uri_y = os.path.join(temp_dir, "y" + array_uuid)
    ...
    def test_except_with_diff_number_of_batch_x_y_rows_empty_record(
        ...
        with temp_uri as temp_dir:
            ...
            tiledb_uri_x = os.path.join(temp_dir, "x")
            tiledb_uri_y = os.path.join(temp_dir, "y")
